...
function resize(init) {
  ...
  //map0.fitBounds(bound, {speed:10000});
  map0.jumpTo({zoom:zm_lv});
...
var finish_path = function(){
 //need to add the filter to use the chapter along with type selected
...
var initContent = function() {
  ...
  for (k in media) {
    console.log(media[k]);
    if (media[k].indexOf(".JPG") > -1 ||media[k].indexOf(".jpg") > -1 || media[k].indexOf(".png") > -1 || media[k].indexOf(".gif") > -1) {
      img.push(media[k]);
...
var updateOverall = function(places){
  //////////////////////update the box info for current path overall./////////////////////////////////

  $("#routeinfobox").show();
  d3.select("#box_title").text(getNode(places, 0)[3].toUpperCase());
  d3.select("#box_id").text($("#nowpath_title").text());
  d3.select("#box_summary").text(getNode(places, 0)[9]);

  d3.selectAll("#box_rt_summary div").remove();

  var placesize = Object.size(places);

  var thismedia = [];

  var thiscount = 1;
  for(k in places){
    if(places[k][3].length>0){

      var thisinfo = {}
      thisinfo.thistitle = places[k][3];
      thisinfo.thisstory = places[k][4];
      thisinfo.thislink = places[k][5];
      thisinfo.thistype = places[k][6];
      thisinfo.thisdis = places[k][8];

      thismedia.push(thisinfo);
      var thiscontent = d3.select("#box_rt_summary").append("div").attr("class", "sub_summary");

      /////////////////////need to fix for video
      thiscontent.append("img").attr("src", places[k][5].split(",")[0]).attr("class", "sub_img");
      thiscontent.append("p").text("Stop "+ thiscount).attr("class", "sub_title");
      thiscontent.append("p").text(places[k][4]).attr("class", "sub_des");
      thiscount ++;
...
var updateContent = function(num) {
  ...
  $("#Coord p").fadeOut(0, function() {
    $(this).remove()
    d3.select("#Coord")
      .append("p")
      .text("Coord");
    $("#Coord p").fadeOut(0).fadeIn(0);
  });
  ...
  $("#Coordnum p").fadeOut(500, function() {
    ...
    d3.select("#Coordnum")
      .append("p")
      .text(getNode(places, num)[0].toString().substr(0,7) + "  |  " + getNode(places, num)[1].toString().substr(0,6));
    $("#Coordnum p").fadeOut(0).fadeIn(500);
  });

  $("#number p").fadeOut(0, function() {
    var strnum = nowNum;
